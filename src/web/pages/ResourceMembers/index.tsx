...
const ResourceMembersPerEvent = React.lazy(
  () => import("../ResourceMembersPerEvent")
);
const ResourceMembersPerGroup = React.lazy(
  () => import("../ResourceMembersPerGroup")
);

const ResourceMembers: React.FC = (): JSX.Element => {
  const [resourceType, setResourceType] = useState<any>("group");

  const isMembersPerResourceLoading = useStoreState(
    (state) => state.dashboard.isMembersPerResourceLoading
  );
    ...
    <>
      <div className="col-lg-6">
        <div className="cardBox dashboardBoxes">
          ...
          {resourceType === "group" ? (
            <ResourceMembersPerGroup />
          ) : (
            <ResourceMembersPerEvent />
          )}
        </div>
      </div>
    </>
