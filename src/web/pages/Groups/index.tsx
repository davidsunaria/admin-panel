...
const Groups: React.FC = (): JSX.Element => {
  ...
  const isLockedUnlocked = useStoreState(
    (state) => state.group.isLockedUnlocked
  );
  ...
  useEffect(() => {
    async function changeData() {
      ...
      localStateData[index].status = currentGroupStatus === "delete" ? 0 : 1;
  ...
  const renderStatusRow = (val: any) => {
    return (
      ...
      >
        <div
          onClick={() => manageAction(val?._id, val?.status)}
          className={
